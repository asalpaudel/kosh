import type { SVGProps } from "react";

export function Logo({ className }: SVGProps<SVGSVGElement>) {
  return (
    <svg width="30" height="30" viewBox="0 0 189 235" fill="none" xmlns="http://www.w3.org/2000/svg" className={className}>
      <path d="M54.6339 159.688C54.6339 161.055 54.1681 162.387 53.3071 163.458L34.2955 187.171L10.7546 218.031L10.7559 218.032C7.2741 222.599 0 220.129 0 214.383V96.8369C0.000311621 72.7101 19.0853 52.9297 43.1379 52.1278L45.3102 52.0548V44.7339C45.3102 20.0281 65.2876 0 89.9309 0H147.257C151.983 0 154.788 5.18751 152.382 9.12569L152.134 9.50256L118.563 56.6098C117.547 58.0358 115.964 58.9419 114.233 59.1005L113.885 59.1227L54.6339 61.097V159.688ZM9.32374 204.521L26.9658 181.394L27.0009 181.349L45.3102 158.51V61.4074L43.4475 61.47C24.4209 62.1044 9.32405 77.7516 9.32374 96.8369V204.521ZM54.6339 51.7444L111.932 49.834L140.786 9.34738H89.9309C70.437 9.34738 54.6339 25.1905 54.6339 44.7339V51.7444Z" fill="#3EEFB1" />
      <path d="M133.138 183.254L75.2937 185.163L46.1673 225.653H97.5052C117.185 225.653 133.138 209.81 133.138 190.266V183.254ZM178.879 30.478L161.104 53.5622L161.068 53.6078L161.033 53.6509L142.551 76.4872V173.591L144.43 173.53C163.638 172.895 178.879 157.249 178.879 138.163V30.478ZM188.292 138.163C188.292 162.291 169.026 182.07 144.743 182.872L142.551 182.944V190.266C142.551 214.973 122.383 235 97.5052 235H39.6332C34.7126 235 31.8479 229.477 34.709 225.499L68.5994 178.39L68.6007 178.389C69.6989 176.864 71.4439 175.944 73.3122 175.879L73.3214 175.877L133.138 173.902V75.3123C133.138 73.9417 133.610 72.6179 134.464 71.5566L134.472 71.5475L153.669 47.8283L177.435 16.9669L177.603 16.7569C181.179 12.4802 188.292 14.9612 188.292 20.6169V138.163Z" fill="#3EEFB1" />
    </svg>
  );
}

export { FiFilter as FunnelIcon, FiSearch as SearchIcon, FiBell as BellIcon, FiSettings as SettingsIcon, FiGrid as LayoutDashboardIcon, FiUsers as UsersIcon, FiMail as MailIcon, FiBarChart2 as BarChartIcon, FiFileText as FileTextIcon, FiPlusCircle as PlusCircleIcon, FiLogOut as LogOutIcon, FiEye as EyeIcon, FiEdit2 as PencilIcon, FiTrash2 as TrashIcon, FiX as CloseIcon, FiBriefcase as BuildingIcon, FiUser as UserCircleIcon, FiPhone as PhoneIcon, FiFile as DocumentIcon, FiCheck as CheckIcon, FiX as XIcon, FiUserPlus as UserPlusIcon, FiActivity as ActivityIcon, FiFeather as LeafIcon, FiCalendar as CalendarIcon, FiClipboard as ClipboardListIcon, FiCheckCircle as CheckCircleIcon, FiXCircle as XCircleIcon, FiClock as ClockIcon, FiMoon as MoonIcon, FiShield as ShieldIcon, FiUser as UserIcon, FiX as XMarkIcon } from "react-icons/fi";
export { FaPiggyBank as PiggyBankIcon, FaPiggyBank as CurrencyDollarIcon, FaFileSignature as BanknotesIcon } from "react-icons/fa";
export { LuFileClock as DocumentTextIcon } from "react-icons/lu";
