...
public interface TransactionRepository extends JpaRepository<Transaction, Long> {
    ...
    Optional<Transaction> findByIdForApproval(@Param("id") Long id);
    ...
    @Query("SELECT COALESCE(SUM(CASE WHEN t.direction = 'Credit' THEN t.amount ELSE -t.amount END), 0) " +
           "FROM Transaction t WHERE t.network.id = :networkId AND t.accountHead = :head AND t.status = 'Success'")
    ...
    @Query("SELECT COALESCE(SUM(CASE WHEN t.direction = 'Debit' THEN t.amount ELSE -t.amount END), 0) " +
           "FROM Transaction t WHERE t.network.id = :networkId AND t.accountHead = 'Loan' AND t.status = 'Success'")
    ...
    @Query("SELECT COALESCE(SUM(CASE WHEN t.direction = 'Credit' THEN t.amount ELSE -t.amount END), 0) " +
           "FROM Transaction t WHERE t.network.id = :networkId AND t.mode = 'network' AND t.status = 'Success'")
