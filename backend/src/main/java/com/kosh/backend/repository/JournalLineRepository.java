...
public interface JournalLineRepository extends JpaRepository<JournalLine, Long> {
    ...
    /** Trial balance rows: account code, name, type, total debits, total credits. */
    @Query("""
           ...
           WHERE a.network.id = :networkId AND e.entryDate <= :asOf
           GROUP BY a.code, a.name, a.type
           ORDER BY a.code
           """)
    List<Object[]> trialBalance(@Param("networkId") Long networkId, @Param("asOf") LocalDate asOf);
}
